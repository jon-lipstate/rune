...
create_feature_set :: proc "contextless" (tags: ..Feature_Tag) -> Feature_Set {
	s := Feature_Set{}
	for t in tags {feature_set_add(&s, t)}
	return s
}

feature_set_add :: proc "contextless" (set: ^Feature_Set, tag: Feature_Tag) {
	array_idx, bit_pos := feature_set_indexer(tag)
	set.bits[array_idx] |= 1 << bit_pos
}

// Remove a tag from the set
feature_set_remove :: proc "contextless" (set: ^Feature_Set, tag: Feature_Tag) {
	array_idx, bit_pos := feature_set_indexer(tag)
	set.bits[array_idx] &= ~(1 << bit_pos)
}
...
feature_set_indexer :: proc "contextless" (tag: Feature_Tag) -> (array_index: int, bit_position: uint) {
	#partial switch tag {
	case .aalt:
		return 0, 1
	case .abvf:
