...
GRAPHEME_TEST :: #directory + "ucd/GraphemeBreakTest.txt"

// Same format and the same reasoning as the line-break suite: the Consortium
// ships the tests, so a percentage here is a fact.
@(test)
grapheme_clusters_match_the_unicode_conformance_suite :: proc(t: ^testing.T) {
	data, err := os.read_entire_file_from_path(GRAPHEME_TEST, context.allocator)
	if err != nil {
		testing.fail_now(t, "ucd/GraphemeBreakTest.txt missing -- see text/tools/")
	}
	defer delete(data)
	...
	for line in strings.split_lines_iterator(&rest) {
		...
		it := into_grapheme_iterator(text)
		for {
			off, ok := next_grapheme(&it)
			if !ok {break}
			got[off] = true
		}
